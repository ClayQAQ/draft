...
public class RKDiskCache {
 ...
 extension RKDiskCache {
  ...
  public static func append<T: Codable>(_ value: T, to path: String, in directory: Directory, decoder: JSONDecoder = JSONDecoder(), encoder: JSONEncoder = JSONEncoder()) throws {
        if path.hasSuffix("/") {
            throw createInvalidFileNameForStructsError()
        }
        do {
...
extension RKDiskCache {
    ...
    fileprivate static func createDeserializationErrorForAppendingStructToInvalidType<T>(url: URL, type: T) -> Error {
        return RKDiskCache.createError(
            .deserialization,
