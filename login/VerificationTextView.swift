...
class VerificationTextView: UIView {
    
    /// 光标颜色
    var cursorColor = UIColor.init(HEX: 0x3C81F6)
    ...
    fileprivate lazy var lineView: UIView = {
        let line = UIView()
        line.backgroundColor = UIColor.init(HEX: 0x3B3B3B)
...
// MARK: - 供外部调用方法
extension VerificationTextView {
    /// 设置光标是否隐藏
    ///
    ...
    func setBottomLineFocus(isFocus: Bool) {
        if isFocus {
            lineView.backgroundColor = UIColor.init(HEX: 0x999999)
        } else {
            lineView.backgroundColor = UIColor.init(HEX: 0xe6e6e6)
        }
    }
