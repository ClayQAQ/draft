...
class VerificationCodeView: UIView {
    ...
    /// 验证码数量
    var codeViews: [VerificationTextView] = []
    ...
    func initSubviews() {
        ...
        // 每个验证码框宽度
        let totalWidth: CGFloat = UIScreen.width() - padding * 2 - spacing * (CGFloat(inputTextNum) - 1)
        let itemWidth = totalWidth / CGFloat(inputTextNum)
        for i in 0..<inputTextNum {
            let codeNumView = VerificationTextView()
